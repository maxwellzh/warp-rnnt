...
__global__ void kernel_fill_costs(float *costs, float *grads,
  ...
  float a =
      alphas[idx3(n, t, u, T, U)] + log_probs[idx4(n, t, u, blank, T, U, V)];
...
void run_warp_rnnt(unsigned int *counts, float *alphas, float *betas,
  ...
  kernel_grads_blank<<<blocks2, G>>>(grads, alphas, betas, log_probs, xn, yn, T,
                                     U, V, blank);
  ...
  if (U > 1) {
    ...
    kernel_grads_label<<<blocks3, G>>>(grads, alphas, betas, labels, log_probs,
                                       xn, yn, T, U, V, fastemit_lambda);
    CHECK_KERNEL_STAT("rnnt loss computing grad for labels")
  }

  dim3 blocks4((N + B - 1) / B, 1, 1);
  kernel_fill_costs<<<blocks4, B>>>(costs, grads, alphas, betas, log_probs, xn,
                                    yn, N, T, U, V, blank);
