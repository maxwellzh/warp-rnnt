...
__forceinline__ __device__ static int idx4(int n, int t, int u, int v, int T,
                                           int U, int V) {
...
#pragma unroll
    ...
    if (t < actual_t && u < actual_u) {
        ...
        float emit = alphas[idx3(n, t, u - 1, T, U)] +
                     log_probs[idx4(n, t, u - 1, l, T, U, V)];
...
__device__ void kernel_warp_betas(unsigned int *counts, volatile float *betas,
    ...
    if (blockIdx.x == 0 && blockIdx.y == 0) {
        betas[idx3(n, T1, U1, T, U)] =
            log_probs[idx4(n, T1, U1, blank, T, U, V)];
    }
...
#pragma unroll
    ...
    if (t < actual_t && u < actual_u) {
        ...
        float emit = betas[idx3(n, T1 - t, U1 - u + 1, T, U)] +
                     log_probs[idx4(n, T1 - t, U1 - u, l, T, U, V)];
...
__global__ void kernel_warp(unsigned int *counts, volatile float *alphas,
                            volatile float *betas, const int *labels,
                            const float *log_probs, const int *xn,
                            const int *yn, int T, int U, int V, int blank) {
    if (threadIdx.y == 0) {
        kernel_warp_alphas(counts, alphas, labels, log_probs, xn, yn, T, U, V,
                           blank);
    } else if (threadIdx.y == 1) {
        kernel_warp_betas(counts, betas, labels, log_probs, xn, yn, T, U, V,
                          blank);
    }
}

__global__ void kernel_grads_blank(float *grads, const float *alphas,
...
__global__ void kernel_grads_label(float *grads, const float *alphas,
                                   const float *betas, const int *labels,
                                   const float *log_probs, const int *xn,
                                   const int *yn, int T, int U, int V,
...
__global__ void kernel_fill_costs(float *costs, float *grads,
                                  const float *alphas, const float *betas,
                                  const float *log_probs, const int *xn,
                                  const int *yn, int N, int T, int U, int V,
                                  int blank) {
...
void run_warp_rnnt(unsigned int *counts, float *alphas, float *betas,
    ...
    kernel_warp<<<blocks1, threads1>>>(counts, alphas, betas, labels, log_probs,
                                       xn, yn, T, U, V, blank);
    CHECK_KERNEL_STAT("rnnt loss computing alpha/beta")

    dim3 blocks2((T + G - 1) / G, U, N);
    kernel_grads_blank<<<blocks2, G>>>(grads, alphas, betas, log_probs, xn, yn,
                                       T, U, V, blank);
    CHECK_KERNEL_STAT("rnnt loss computing grad for blank")

    if (U > 1) {

        dim3 blocks3((T + G - 1) / G, U - 1, N);
        kernel_grads_label<<<blocks3, G>>>(grads, alphas, betas, labels,
                                           log_probs, xn, yn, T, U, V,
                                           fastemit_lambda);
        CHECK_KERNEL_STAT("rnnt loss computing grad for labels")
    }

    dim3 blocks4((N + B - 1) / B, 1, 1);
    kernel_fill_costs<<<blocks4, B>>>(costs, grads, alphas, betas, log_probs,
                                      xn, yn, N, T, U, V, blank);
    CHECK_KERNEL_STAT("rnnt loss computing costs")
