...
#pragma unroll
  ...
  if (t < actual_t && u < actual_u) {
    ...
    float emit = alphas[idx3(n, t, u - 1, T, U)] +
                 log_probs[idx4(n, t, u - 1, 1, T, U, 2)];
...
#pragma unroll
  ...
  if (t < actual_t && u < actual_u) {
    ...
    float emit = betas[idx3(n, T1 - t, U1 - u + 1, T, U)] +
                 log_probs[idx4(n, T1 - t, U1 - u, 1, T, U, 2)];
...
__global__ void kernel_warp(unsigned int *counts, volatile float *alphas,
                            volatile float *betas, const float *log_probs,
...
__global__ void kernel_fill_costs(float *costs, float *grads,
                                  ...
                                  const float *log_probs, const int *xn,
                                  const int *yn, int N, int T, int U) {
...
void run_warp_rnnt_gather(unsigned int *counts, float *alphas, float *betas,
  ...
  kernel_warp<<<blocks1, threads1>>>(counts, alphas, betas, log_probs, xn, yn,
                                     T, U);
  ...
  if (U > 1) {
    ...
    kernel_grads_label<<<blocks3, G>>>(grads, alphas, betas, log_probs, xn, yn,
                                       T, U, fastemit_lambda);
