...
kernel_warp_alphas_compact(unsigned int *counts, volatile float *alphas,
  ...
  if (blockIdx.x == 0 && u < actual_u) {
    ...
    float b =
        log_probs[mem_beg + (u << 1) - 1]; // should be [mem_beg + 2 * (u-1) +
...
__global__ void kernel_warp_compact(
  ...
  if (threadIdx.y == 0) {
    kernel_warp_alphas_compact(counts, alphas, log_probs, xn, yn, memPref,
                               labelPref);
  } else if (threadIdx.y == 1) {
    kernel_warp_betas_compact(counts, betas, log_probs, xn, yn, memPref,
                              labelPref);
  }
}
...
void run_warp_rnnt_compact_gather(
  ...
  kernel_warp_compact<<<blocks1, threads1>>>(counts, alphas, betas, log_probs,
                                             xn, yn, memPref, labelPref);
  CHECK_KERNEL_STAT("rnnt loss compact gather")
  ...
  if (U > 1) {
    ...
    kernel_grads_label_compact<<<blocks3, G>>>(grads, alphas, betas, log_probs,
                                               xn, yn, memPref, labelPref,
                                               fastemit_lambda);
    CHECK_KERNEL_STAT("rnnt loss compact computing gradients for labels")
  }

  dim3 blocks4((N + B - 1) / B, 1, 1);
  kernel_fill_costs_compact<<<blocks4, B>>>(costs, grads, alphas, betas,
                                            log_probs, xn, yn, memPref, N);
