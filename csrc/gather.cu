...
void run_gather(const float *xs, const int *ys, const unsigned int *xn,
  ...
  kernel_fill_gather<<<blocks1, threads1>>>(xs, ys, xn, yn, gather_xs, loc,
                                            memPref, labelPref, V, blank);
  CHECK_KERNEL_STAT("rnnt loss gathering")
...
void run_scatter_grad(const float *grad_cost, const float *gather_grad,
  ...
  kernel_fill_scatter_grad<<<blocks1, threads1>>>(
      grad_cost, gather_grad, loc, cumSum, scatter_grad, STU, V, blank);
  CHECK_KERNEL_STAT("rnnt loss filling scatter grad")

  return;
}

__global__ void kernel_fill_grad(const float *grad_cost, float *grad,
...
void run_backward_compact(const float *grad_cost, float *grad,
  ...
  kernel_fill_grad<<<blocks, threads>>>(grad_cost, grad, cumSum, STU, V);
  CHECK_KERNEL_STAT("rnnt loss filling grad")
