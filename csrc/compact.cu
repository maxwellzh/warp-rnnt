...
__global__ void kernel_grads_label_compact(
    ...
    // index = (n, t, u, 1);
    unsigned int index = ((mem_loc + _index) << 1) + 1;
...
__global__ void
kernel_fill_scatter_grad(const float *grad_cost, const float *gather_grad,
                         const long *loc, const unsigned int *cumSum,
                         float *scatter_grad, unsigned int STU, unsigned int V,
                         unsigned int N, unsigned int blank) {
...
void run_scatter_grad_compact(const float *grad_cost, const float *gather_grad,
    ...
    kernel_fill_scatter_grad<<<blocks, threads>>>(
        grad_cost, gather_grad, loc, cumSum, scatter_grad, STU, V, N, blank);
