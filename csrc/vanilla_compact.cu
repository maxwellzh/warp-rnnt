...
__device__ void kernel_warp_alphas_compact(
    unsigned int *counts, volatile float *alphas, const unsigned int *labels,
    const float *log_probs, const unsigned int *xn, const unsigned int *yn,
    const unsigned int *memPref, const unsigned int *labelPref, unsigned int V,
    unsigned int blank) {
...
#pragma unroll
  ...
  if (t < actual_t && u < actual_u) {
    ...
    float emit = alphas[mem_loc + t * actual_u + u - 1] +
                 log_probs[mem_beg + t * actual_u * V + (u - 1) * V + l];
...
__device__ void kernel_warp_betas_compact(
    unsigned int *counts, volatile float *betas, const unsigned int *labels,
    const float *log_probs, const unsigned int *xn, const unsigned int *yn,
    const unsigned int *memPref, const unsigned int *labelPref, unsigned int V,
    unsigned int blank) {
  ...
  if (blockIdx.x == 0 && blockIdx.y == 0) {
    ...
    betas[mem_loc + T1 * actual_u + U1] =
        log_probs[mem_beg + T1 * actual_u * V + U1 * V + blank];
  }
...
#pragma unroll
  ...
  if (t < actual_t && u < actual_u) {
    ...
    float bias =
        log_probs[mem_beg + (T1 - t) * actual_u * V + (U1 - u) * V + blank];
    ...
    float emit =
        betas[mem_loc + (T1 - t) * actual_u + U1 - u + 1] +
        log_probs[mem_beg + (T1 - t) * actual_u * V + (U1 - u) * V + l];
...
__global__ void
kernel_alpha_only(unsigned int *counts, volatile float *alphas,
  ...
  kernel_warp_alphas_compact(counts, alphas, labels, log_probs, xn, yn, memPref,
                             labelPref, V, blank);
}

__global__ void kernel_warp_compact(
    unsigned int *counts, volatile float *alphas, volatile float *betas,
    const unsigned int *labels, const float *log_probs, const unsigned int *xn,
    const unsigned int *yn, const unsigned int *memPref,
    const unsigned int *labelPref, unsigned int V, unsigned int blank) {
  if (threadIdx.y == 0) {
    kernel_warp_alphas_compact(counts, alphas, labels, log_probs, xn, yn,
                               memPref, labelPref, V, blank);
  } else if (threadIdx.y == 1) {
    kernel_warp_betas_compact(counts, betas, labels, log_probs, xn, yn, memPref,
                              labelPref, V, blank);
  }
}

__global__ void kernel_grads_blank_compact(
...
__global__ void kernel_grads_label_compact(
    ...
    const unsigned int *labels, const float *log_probs, const unsigned int *xn,
    const unsigned int *yn, const unsigned int *memPref,
    const unsigned int *labelPref, unsigned int V, float fastemit_lambda) {
...
__global__ void kernel_fill_cost_with_alpha(
    float *costs, const float *alphas, const float *log_probs,
  ...
  float a = alphas[mem_loc + t * (u + 1) + u] +
            log_probs[mem_beg + t * (u + 1) * V + u * V + blank];

  costs[n] = -a;
}

__global__ void
kernel_fill_costs_compact(float *costs, float *grads, const float *alphas,
                          const float *betas, const float *log_probs,
                          const unsigned int *xn, const unsigned int *yn,
   ...
   * ... at the position (t, u). So here is a assertion whether these two
   * variables
  ...
  if (ratio > 0.01) {
    ...
    printf("\nWARNING: sample %d [%d, %d] has a forward/backward mismatch %f / "
           "%f\n",
           n, t + 1, u, a, b);
...
void run_rnnt_cost_cal_compact(unsigned int *counts, float *alphas,
  ...
  kernel_alpha_only<<<blocks1, threads1>>>(
      counts, alphas, labels, log_probs, xn, yn, memPref, labelPref, V, blank);
  CHECK_KERNEL_STAT("rnnt loss computing alpha")

  dim3 blocks4((N + B - 1) / B, 1, 1);
  kernel_fill_cost_with_alpha<<<blocks4, B>>>(costs, alphas, log_probs, xn, yn,
                                              memPref, N, V, blank);
  CHECK_KERNEL_STAT("rnnt loss computing cost")
  return;
}
...
void run_warp_rnnt_compact(unsigned int *counts, float *alphas, float *betas,
  ...
  dim3 threads1(W, 2);
  dim3 blocks1((Tm + W - 1) / W, Um, N);
  kernel_warp_compact<<<blocks1, threads1>>>(counts, alphas, betas, labels,
                                             log_probs, xn, yn, memPref,
                                             labelPref, V, blank);
  CHECK_KERNEL_STAT("rnnt loss computing alpha/beta")

  dim3 blocks2((Tm + G - 1) / G, Um, N);
  kernel_grads_blank_compact<<<blocks2, G>>>(grads, alphas, betas, log_probs,
                                             xn, yn, memPref, V, blank);

  CHECK_KERNEL_STAT("rnnt loss computing gradients for blank")

  if (Um > 1) {
    dim3 blocks3((Tm + G - 1) / G, Um - 1, N);
    kernel_grads_label_compact<<<blocks3, G>>>(grads, alphas, betas, labels,
                                               log_probs, xn, yn, memPref,
                                               labelPref, V, fastemit_lambda);
    CHECK_KERNEL_STAT("rnnt loss computing gradients for labels")
  }

  dim3 blocks4((N + B - 1) / B, 1, 1);
  kernel_fill_costs_compact<<<blocks4, B>>>(
      costs, grads, alphas, betas, log_probs, xn, yn, memPref, N, V, blank);
  CHECK_KERNEL_STAT("rnnt loss computing cost")
...
void run_alphabeta_div_prob(float *alphabetas, const float *costs,
  ...
  dim3 blocks2((Tm + G - 1) / G, Um, N);
  kernel_fill_fused<<<blocks2, G>>>(alphabetas, costs, xn, yn, memPref);
  CHECK_KERNEL_STAT("rnnt loss fused computing alpha*beta/prob")
